...
def test_process_registration_success():
    ...
    serialized_credential = server.process_registration(sk, pk, serialized_registration, "user", user_subscriptions)

    # Process the response
    credentials = client.process_registration_response(pk, serialized_credential, state)

    # Sign a request
    signed_request = client.sign_request(pk, credentials, b"message", ["C"])

    # Verify the signature
    assert server.check_request_signature(pk, b"message", ["C"], signed_request)
