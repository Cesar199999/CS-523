...
def test_create_sign_unblind_signature_success():
    ...
    issuer_attributes = {i: i.to_bytes(1, "big") for i in set(range(6)) - user_attributes.keys()}

    # Create an issue request
    issue_request = create_issue_request(pk, user_attributes, user_state)

    # Sign the issue request
    signed_issue_request = sign_issue_request(sk, pk, issue_request, issuer_attributes)

    # Obtain the credential
    credential_signature, credentials = obtain_credential(pk, signed_issue_request, user_state)

    sorted_credentials = [credentials[i] for i in sorted(credentials.keys())]
...
def test_create_sign_unblind_signature_failure_on_credential_signature_modification():
    ...
    malicous_credential_signature = G1.neutral_element(), credential_signature[1]
...
def test_showing_protocol_success_1():
    ...
    # Obtain the credential
    credential = obtain_credential(pk, signed_issue_request, user_state)

    # Create a disclosure proof
    proof = create_disclosure_proof(pk, credential, [b"A"], b"test")
...
def test_showing_protocol_failure_on_credential_modification():
    ...
    # Obtain the credential
    credential_signature, _ = obtain_credential(pk, signed_issue_request, user_state)

    # Modify the credentials
    credentials = {0: b"error", 1: b"error", 2: b"error", 3: b"error", 4: b"error", 5: b"error"}

    # Create a disclosure proof
    proof = create_disclosure_proof(pk, (credential_signature, credentials), [b"error"], b"test")
...
def test_showing_protocol_failure_on_credential_left_signature_modification():
    ...
    # Create a disclosure proof
    proof = create_disclosure_proof(pk, (malicous_credential_signature, credentials), [b"A"], b"test")
